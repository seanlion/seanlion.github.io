...
서비스를 설계하기에 필요한 상호작용, 페르소나, 환경을 살펴보았다. 사실 저 모든 것들을 파악하기란 어렵다. 이렇게 설명은 했지만 나 또한 실제로 매번 저런 분석을 할 수는 없을 것이다. 하지만 분석을 잘 수행한다면 분명 좋은 서비스는 나올 것이라 확신한다.(다음번에 서비스를 분석하는 글을 올린다면 저 질문들을 답하는 분석기를 올려보도록 하겠다.) 